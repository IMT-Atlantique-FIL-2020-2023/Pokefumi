...
import { getNumberOfRoundsByPokemonPathDeserializer } from '../deserializers/getNumberOfRoundsByPokemonPathDeserializer';
import { GetNumberOfRoundsByPokemonServerRequest } from '../requests/GetNumberOfRoundsByPokemonServerRequest';

export const getNumberOfRoundsByPokemonRouter: Router = Router().get(
  ...
  async (request: Request, response: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      const path = await configuration.getPathParameters(toolkit, getNumberOfRoundsByPokemonPathDeserializer);
      const typedRequest: GetNumberOfRoundsByPokemonServerRequest = {
        path,
      };
      const typedResponse = await api.getNumberOfRoundsByPokemon(typedRequest, toolkit);
